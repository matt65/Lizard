...
class SunriseSunset(object):
    ...
    def calculate(self, date=None):
        ...
        t_rise = float(N + ((6 - lngHour) / 24))
        t_set = float(N + ((18 - lngHour) / 24))

        # Calculate the Sun's mean anomaly
        M_rise = (float(0.9856) * t_rise) - float(3.289)
        M_set = (float(0.9856) * t_set) - float(3.289)

        # Calculate the Sun's true longitude, and adjust angle to be between 0
        # and 360
        L_rise = (M_rise + (float(1.916) * float(math.sin(math.radians(M_rise)))) + (float(0.020) * float(math.sin(math.radians(2 * M_rise)))) + float(282.634)) % 360
        L_set = (M_set + (float(1.916) * float(math.sin(math.radians(M_set)))) + (float(0.020) * float(math.sin(math.radians(2 * M_set)))) + float(282.634)) % 360
        ...
        RA_rise = float(RA_rise / 15)
        RA_set = float(RA_set / 15)
        ...
        H_rise = float((360 - math.degrees(math.acos(cosH_rise))) / 15)
        H_set = float(math.degrees(math.acos(cosH_set)) / 15)

        # Calculate local mean time of rising/setting
        T_rise = H_rise + RA_rise - (float(0.06571) * t_rise) - float(6.622)
        T_set = H_set + RA_set - (float(0.06571) * t_set) - float(6.622)

        # Adjust back to UTC, and keep the time between 0 and 24
        UT_rise = (T_rise - float(lngHour)) % 24
        UT_set = (T_set - float(lngHour)) % 24
